...
def get_args():
    ...
    parser.add_argument("--save_tensors", type=str, default=None, help="Directory where to save tensors")
...
def eval_ood_md2sonn(opt, config):
    ...
    if opt.save_tensors:
        if not osp.exists(opt.save_tensors):
            os.makedirs(opt.save_tensors)
        torch.save(src_logits,  opt.save_tensors + '/src_logits.pt')
        torch.save(src_pred,    opt.save_tensors + '/src_pred.pt')
        torch.save(src_labels,  opt.save_tensors + '/src_labels.pt')
        torch.save(tar1_logits, opt.save_tensors +'/tar1_logits.pt')
        torch.save(tar2_logits, opt.save_tensors +'/tar2_logits.pt')
        print(f"Logits saved to {opt.save_tensors}")
    # MSP
    print("\n" + "#" * 80)
    ...
    # FEATURES EVALUATION
    eval_OOD_with_feats(model, train_loader, id_loader, ood1_loader, ood2_loader, save_feats=opt.save_feats, save_tensors=opt.save_tensors)
...
def eval_OOD_with_feats(model, train_loader, src_loader, tar1_loader, tar2_loader, save_feats=None, save_tensors=None):
    ...
    if save_tensors is not None:
        if not osp.exists(save_tensors):
            os.makedirs(save_tensors)
        torch.save(src_dist,  save_tensors + '/src_dist.pt')
        torch.save(src_ids,   save_tensors + '/src_ids.pt')
        torch.save(tar1_dist, save_tensors + '/tar1_dist.pt')
        torch.save(tar1_ids,  save_tensors + '/tar1_ids.pt')
        torch.save(tar2_dist, save_tensors + '/tar2_dist.pt')
        torch.save(tar2_ids,  save_tensors + '/tar2_ids.pt')
        print(f"Euclid L2 distances saved to {save_tensors}")
